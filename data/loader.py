"""
Data loader for enhanced TACRED json files
...
class Entry(namedtuple('Entry',
                       'token, pos, ner, coref, head, ucca_head, ucca_multi_head, subj_p, obj_p, ucca_enc, ucca_dist_from_mh_path, rel, id')):
    """
...
class Batch(namedtuple('Batch',
                       'batch_size, word, pos, ner, coref, head, ucca_head, ucca_multi_head, subj_p, obj_p, ucca_enc, ucca_dist_from_mh_path, rel, orig_idx, id, len')):
    """
...
class DataLoader(object):
    ...
    def preprocess(self, data, vocab, opt):
        ...
        for d in data:
            ...
            if len(d['ucca_heads']) != len(d['corenlp_heads']):
                print(
                    'Skipping document {} as it exhibits a discrepancy between UCCA tokenization and CoreNLP tokenization'.format(
                        d['id']))
                continue
            ...
            if opt['ucca_embedding_dim'] > 0:
                ...
                ucca_embedding_source = 'ucca_encodings_min_subtree' if opt[
                                                                            'ucca_embedding_source'] == 'min_sub_tree' else 'ucca_encodings'

                assert (ucca_embedding_source in d)

                index_to_encoding = {int(k): v for k, v in d[ucca_embedding_source].items()} if d[
                                                                                                    ucca_embedding_source] is not None else {}
                ucca_encodings_for_min_subtree = []
